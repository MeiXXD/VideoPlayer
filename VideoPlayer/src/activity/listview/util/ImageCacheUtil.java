package activity.listview.util;
 ...
 * Date: 16/3/16
 * Time: 10:34
 * Description: 图片缓存帮助类,包含内存缓存LruCache和磁盘缓存DiskLruCache
 */
public class ImageCacheUtil implements ImageCache {
    ...
    public ImageCacheUtil() {
        ...
        try {
            // 获取DiskLruCahce对象
            mDiskLruCache = DiskLruCache.open(getDiskCacheDir(AppController.getInstance(), "VideoPlayer_Cache"), getAppVersion(AppController.getInstance()), 1, DISKMAXSIZE);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
